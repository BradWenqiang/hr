...
<body class="full" id="content">
	...
	<div class="row">
		...
		<form class="form-inline" role="form" id="selectDepVal">
				<div class="row">
					...
					<button type="button" class="btn btn-info btn-sm " id="addEmp"
							>
							<span class="glyphicon glyphicon-user"></span> 新增应聘
						</button>
						 </sec:authorize>
					</div>
				</div>
	...
	var authority = "${authority}";
		...
		$("#addEmp").click(function(){
			 $("#addEmpList").load("WEB-ROOT/html/addEmploy.jsp",function(){
				$("#addModel").modal({
	     	        keyboard: true
	     	    });
		...
		var pageSize = 8;
		var dataUrl = "${pageContext.request.contextPath}/employee";
		selectList= function(){
			...
			myTable = $
					.lTable(
							'#tableList',
							{
								...
								name : [ "应聘编号", "姓名", "年龄", "学历","应聘职位","投递时间","_opt" ],
		...
		//编辑
			...
			$
					.ajax({
						type : "POST", 
						url : "${pageContext.request.contextPath}/employeeSelect", 
						data : id,
						async : true,
						// dataType:"json",   
						contentType : "application/json;charset=utf-8",
						success : function(msg) {
							...
							$("#alertModel").modal({
							 ...
							 $("#addEmpList").load(
										"WEB-ROOT/html/addEmploy.jsp",
										function() {
											$("#addModel").modal({
												keyboard : true
											});
											var newUpdateDate = new Date();
											var employee = JSON.parse(msg);
											if (employee != null) {
												$("#eid").val(
														employee.eid);
												$("#ename").val(
												...
												$("#job").val(
														employee.job);
												newUpdateDate.setTime(employee.sendtime);
